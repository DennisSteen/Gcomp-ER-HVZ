...
runICE <- function(dataSet, SLvector, ReplaceColumnNames = NA, ReplaceColumnValues = NA, BootStraps = 1, Estimate = TRUE, SAVE = FALSE, name = "NaturalCourse"){
  ...
  Preds[TimePoints == max(TimePoints)] <- mean((as.data.frame(CompleteDataset) %>% filter(dag90 == max(TimePoints), ZCL_REL_NR %in% StatinUsers) %>% dplyr::select(CVE,Death) %>% mutate(EndPoint = Death))$EndPoint)
  ...
  for(B in 1:BootStraps){   
    ...
    for(t in rev(TimePoints[-1])){ #Have to skip the last one
      SS <- NewDataSet$dag90 == t - 90 & NewDataSet$Switch == 0 & NewDataSet$Censor == 0 & NewDataSet$CVE == 0
      
      ## Set modeldata
      ModelData <- NewDataSet %>%
        filter(dag90 == t & Switch == 0 & Censor == 0 & CVE == 0) %>%
...
saveRDS(NaturalCourse,file=paste0(DataLocation,"/NaturalCourse_glm_Bootstrap_Death_StatinOnly.rda"))
...
saveRDS(NoPayments,file=paste0(DataLocation,"/NoPayments_glm_Bootstrap_Death_StatinOnly.rda"))
