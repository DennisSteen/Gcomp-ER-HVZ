for(t in TimePoints[-c(1:2)]){ #Can skip the first 2 since there won't be a switch in those timepoints
  ...
  ModelDataX <- as.data.frame(ModelData %>% 
                                dplyr::select(leeftijd_index,geslacht,BN_INKOMEN_OMS,BN_OPLEIDING_OMS,BN_SOCIALE_KLASSE_OMS
                                              ...
                                              ,paste("adherence",Medications,sep="_")) %>%
  ...
  tryCatch({
    Y <- ModelData$CVE
    assign(paste("model_CVE_",t,sep=""),SuperLearner(Y,ModelDataX,family=binomial(),SL.library = SLvector))
