...
runICE <- function(dataSet, SLvector, ReplaceColumnNames = NA, ReplaceColumnValues = NA, BootStraps = 1, Estimate = TRUE, SAVE = FALSE, name = "NaturalCourse"){
  ...
  Preds[TimePoints == max(TimePoints)] <- mean((as.data.frame(CompleteDataset) %>% filter(dag90 == max(TimePoints)) %>% dplyr::select(CVE,Death) %>% mutate(EndPoint = pmax(CVE,Death)))$EndPoint)
...
saveRDS(NaturalCourse,file=paste0(DataLocation,"/NaturalCourse_glm_Bootstrap_Combined.rda"))
...
saveRDS(NoPayments,file=paste0(DataLocation,"/NoPayments_glm_Bootstrap_Combined.rda"))
