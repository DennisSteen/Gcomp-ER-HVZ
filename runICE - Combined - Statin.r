...
runICE <- function(dataSet, SLvector, ReplaceColumnNames = NA, ReplaceColumnValues = NA, BootStraps = 1, Estimate = TRUE, SAVE = FALSE, name = "NaturalCourse"){
  ...
  Preds[TimePoints == max(TimePoints)] <- mean((as.data.frame(CompleteDataset) %>% filter(dag90 == max(TimePoints), ZCL_REL_NR %in% StatinUsers) %>% dplyr::select(CVE,Death) %>% mutate(EndPoint = pmax(CVE,Death)))$EndPoint)
  ...
  fullDataSetBase <- as.data.frame(dataSet %>%
    ...
    filter(ZCL_REL_NR %in% StatinUsers) %>%
    ...
    mutate(EndPoint = pmax(CVE,Death))
  ...
  for(B in 1:BootStraps){   
    ...
    for(t in rev(TimePoints[-1])){ #Have to skip the last one
      SS <- NewDataSet$dag90 == t - 90 & NewDataSet$Switch == 0 & NewDataSet$Censor == 0
      
      ## Set modeldata
      ModelData <- NewDataSet %>%
        filter(dag90 == t & Switch == 0 & Censor == 0) %>%
        dplyr::select(ZCL_REL_NR, EndPoint) %>%
      ...
      NewDataSet$EndPoint[SS] <- pmax(predict(get(paste("model",name,t,sep="_")),newdata = as.data.frame(model.matrix(~ leeftijd_index + geslacht 
                                                                                                                      + BN_INKOMEN_OMS + BN_OPLEIDING_OMS + BN_SOCIALE_KLASSE_OMS
                                                                                                                      + FKG + DKG + HKG 
                                                                                                                      + AV_INDICATOR + TV_INDICATOR
                                                                                                                      + ER_INDICATOR + HERVERZEKERING_ER + ER_REGELING 
                                                                                                                      + Diabetes + COPD + Reuma + Depressie 
                                                                                                                      + Paying_YP
                                                                                                                      + Class_C03AA:YearPassed + Class_C03DA:YearPassed + Class_C07A:YearPassed + Class_C08:YearPassed + Class_C09A:YearPassed + Class_C09C:YearPassed + Class_C10AA:YearPassed + Class_C10AZ:YearPassed - 1 , NewDataSet[SS,])),type="response")$pred,NewDataSet[SS,"EndPoint"]) ##Step 2
      ...
      Preds[TimePoints == t-90,B] <- mean(NewDataSet[SS,"EndPoint"])
...
saveRDS(NaturalCourse,file=paste0(DataLocation,"/NaturalCourse_glm_Bootstrap_Combined_StatinOnly.rda"))
...
saveRDS(NoPayments,file=paste0(DataLocation,"/NoPayments_glm_Bootstrap_Combined_StatinOnly.rda"))
