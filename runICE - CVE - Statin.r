...
runICE <- function(dataSet, SLvector, ReplaceColumnNames = NA, ReplaceColumnValues = NA, BootStraps = 1, Estimate = TRUE, SAVE = FALSE, name = "NaturalCourse"){
  ...
  Preds[TimePoints == max(TimePoints)] <- mean((as.data.frame(CompleteDataset) %>% filter(dag90 == max(TimePoints), ZCL_REL_NR %in% StatinUsers) %>% dplyr::select(CVE))$CVE)
...
saveRDS(NaturalCourse,file=paste0(DataLocation,"/NaturalCourse_glm_Bootstrap_CVE_StatinOnly.rda"))
...
saveRDS(NoPayments,file=paste0(DataLocation,"/NoPayments_glm_Bootstrap_CVE_StatinOnly.rda"))
