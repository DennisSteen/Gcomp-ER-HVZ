for(t in TimePoints[-c(1:5)]){ #Can skip the first 6 since there won't be a switch in those timepoints
  ModelData <- as.data.frame(CompleteDataset %>%
                               filter(dag90 == t & Switch == 0 & CVE == 0 & Death == 0))
  ...
  tryCatch({
    Y <- ModelData$Censor
    assign(paste("model_Censor_",t,sep=""),SuperLearner(Y,ModelDataX,family=binomial(),SL.library = SLvector))
    saveRDS(get(paste("model_Censor_",t,sep="")),file=paste("L:/afdelingen/Actuariaat_Business/DennisS/PhD/A4/Models/model_Censor_",t,".rda",sep=""))
